...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_FourParams(FParty_PartyJIPResponse_BP, FUniqueNetIdRepl, LocalUserId, UPartyId*, PartyId, bool, Success, int32, DeniedResultCode);
...
// @parameters-not-compatible
// DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FParty_PartyInviteRemovedEx_BP, const FUniqueNetId&, LocalUserId, const IOnlinePartyJoinInfo&, Invitation, EPartyInvitationRemovedReason, Reason);
...
// @parameters-not-compatible
// DECLARE_DYNAMIC_MULTICAST_DELEGATE_FourParams(FParty_PartyRequestToJoinReceived_BP, const FUniqueNetId&, LocalUserId, const FOnlinePartyId&, PartyId, const FUniqueNetId&, RequesterId, const IOnlinePartyRequestToJoinInfo&, Request);
...
private:
    ...
    FDelegateHandle DelegateHandle_OnPartyJIPResponse;
...
public:
    ...
    UPROPERTY(BlueprintAssignable, Category = "Online|Party")
    FParty_PartyJIPResponse_BP OnPartyJIPResponse;
    ...
    // @result-delegates-not-compatible: FOnRequestToJoinPartyComplete(const FUniqueNetId& LocalUserId, const FUniqueNetId& PartyLeaderId, const FDateTime& ExpiresAt, const ERequestToJoinPartyCompletionResult Result)
    // void RequestToJoinParty(FUniqueNetIdRepl LocalUserId, int64 PartyTypeId, FUniqueNetIdRepl Recipient);

    // @generator-incompatible
    // void ClearRequestToJoinParty(const FUniqueNetId& LocalUserId, const FOnlinePartyId& PartyId, const FUniqueNetId& Sender, EPartyRequestToJoinRemovedReason Reason);
    ...
    // void QueryPartyJoinability(const FUniqueNetId& LocalUserId, const IOnlinePartyJoinInfo& OnlinePartyJoinInfo);
    ...
    // @generator-duplicate-needs-picking
    // void RespondToQueryJoinability(const FUniqueNetId& LocalUserId, const FOnlinePartyId& PartyId, const FUniqueNetId& RecipientId, bool bCanJoin, int32 DeniedResultCode);
    ...
    // @generator-incompatible
    // FDelegateHandle AddOnPartyJIPDelegate_Handle(const FOnPartyJIPDelegate& Delegate);
