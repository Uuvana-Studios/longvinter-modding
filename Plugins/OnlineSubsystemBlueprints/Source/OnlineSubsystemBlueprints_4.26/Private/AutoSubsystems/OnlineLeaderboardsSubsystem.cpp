...
void UOnlineLeaderboardsSubsystemReadLeaderboards::Activate()
{
    ...
    if (!Handle->ReadLeaderboards(ConvertArrayElements<FUniqueNetIdRepl, TSharedRef<const FUniqueNetId>>(this->__Store__Players, [](const FUniqueNetIdRepl& Val) { return Val.GetUniqueNetId().ToSharedRef(); }), this->__Store__ReadObject->GetLeaderboard()))
    {
