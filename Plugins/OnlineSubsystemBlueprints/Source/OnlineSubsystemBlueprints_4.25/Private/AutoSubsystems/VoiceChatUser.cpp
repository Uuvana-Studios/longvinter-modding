...
TArray<FString> UVoiceChatUser::GetAvailableInputDevices()
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
    ...
    auto __Result = ConvertArrayElements<FString, FString>(Handle->GetAvailableInputDevices(), [](const FString& Val) { return Val; });

    return __Result;
}

TArray<FString> UVoiceChatUser::GetAvailableOutputDevices()
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
    ...
    auto __Result = ConvertArrayElements<FString, FString>(Handle->GetAvailableOutputDevices(), [](const FString& Val) { return Val; });

    return __Result;
}

void UVoiceChatUser::SetInputDevice(FString InputDevice)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
    ...
    Handle->SetInputDevice(InputDevice);
}

void UVoiceChatUser::SetOutputDevice(FString OutputDevice)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
    ...
    Handle->SetOutputDevice(OutputDevice);
}

FString UVoiceChatUser::GetInputDevice()
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
    ...
    auto __Result = Handle->GetInputDevice();

    return __Result;
}

FString UVoiceChatUser::GetOutputDevice()
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
    ...
    auto __Result = Handle->GetOutputDevice();

    return __Result;
}

FString UVoiceChatUser::GetDefaultInputDevice()
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
    ...
    auto __Result = Handle->GetDefaultInputDevice();

    return __Result;
}

FString UVoiceChatUser::GetDefaultOutputDevice()
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
    ...
    auto __Result = Handle->GetDefaultOutputDevice();

    return __Result;
}

UVoiceChatUserConnect* UVoiceChatUserConnect::Connect(UVoiceChatUser* Subsystem )
{
    ...
    UVoiceChatUserConnect* Node = NewObject<UVoiceChatUserConnect>();
    Node->__Store__Subsystem = Subsystem;
    return Node;
}

void UVoiceChatUserConnect::Activate()
{
    ...
    auto __DelegateHandle_OnVoiceChatConnectComplete = FOnVoiceChatConnectCompleteDelegate::CreateUObject(this, &UVoiceChatUserConnect::HandleCallback_OnVoiceChatConnectComplete);

    Handle->Connect(__DelegateHandle_OnVoiceChatConnectComplete);
}

void UVoiceChatUserConnect::HandleCallback_OnVoiceChatConnectComplete(const FVoiceChatResult& Result)
{
...
UVoiceChatUserDisconnect* UVoiceChatUserDisconnect::Disconnect(UVoiceChatUser* Subsystem )
{
    if (!IsValid(Subsystem))
    {
        UE_LOG(LogOnlineBlueprint, Error, TEXT("VoiceChatUser is not available. It's possible the online subsystem you are using doesn't support it. Alternatively, make sure that you are not attempting to use Online Subsystem Blueprints during the Init event in a GameInstance. Add a 'Delay 0.1' node between the Init event and the rest of your logic if you need to use Online Subsystem Blueprints this early in game startup."));
    ...
    UVoiceChatUserDisconnect* Node = NewObject<UVoiceChatUserDisconnect>();
    Node->__Store__Subsystem = Subsystem;
    return Node;
}

void UVoiceChatUserDisconnect::Activate()
{
    auto Handle = this->__Store__Subsystem->GetHandle();
    if (!this->__Store__Subsystem->IsHandleValid(Handle))
    {
    ...
    auto __DelegateHandle_OnVoiceChatDisconnectComplete = FOnVoiceChatDisconnectCompleteDelegate::CreateUObject(this, &UVoiceChatUserDisconnect::HandleCallback_OnVoiceChatDisconnectComplete);

    Handle->Disconnect(__DelegateHandle_OnVoiceChatDisconnectComplete);
}

void UVoiceChatUserDisconnect::HandleCallback_OnVoiceChatDisconnectComplete(const FVoiceChatResult& Result)
{
    if (!IsValid(this->__Store__Subsystem))
    {
        UE_LOG(LogOnlineBlueprint, Error, TEXT("Subsystem was not valid during callback handling"));
...
bool UVoiceChatUser::IsConnecting()
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
...
bool UVoiceChatUser::IsConnected()
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
