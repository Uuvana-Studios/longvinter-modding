...
UOnlinePartySubsystem::UOnlinePartySubsystem()
{
    ...
    if (Handle.IsValid())
    {
        ...
        this->DelegateHandle_OnPartyJoinRequestReceived = Handle->AddOnPartyJoinRequestReceivedDelegate_Handle(
            FOnPartyJoinRequestReceived::FDelegate::CreateWeakLambda(this, [this](const FUniqueNetId& LocalUserId, const FOnlinePartyId& PartyId, const FUniqueNetId& SenderId, const FString& Platform, const FOnlinePartyData& PartyData) {
                this->OnPartyJoinRequestReceived.Broadcast(ConvertDangerousUniqueNetIdToRepl(LocalUserId), UPartyId::FromNative(PartyId), ConvertDangerousUniqueNetIdToRepl(SenderId), Platform, UReadablePartyData::FromNative(PartyData));
            })
        );
        ...
        this->DelegateHandle_OnQueryPartyJoinabilityReceived = Handle->AddOnQueryPartyJoinabilityReceivedDelegate_Handle(
            FOnQueryPartyJoinabilityReceived::FDelegate::CreateWeakLambda(this, [this](const FUniqueNetId& LocalUserId, const FOnlinePartyId& PartyId, const FUniqueNetId& SenderId, const FString& Platform, const FOnlinePartyData& PartyData) {
                this->OnQueryPartyJoinabilityReceived.Broadcast(ConvertDangerousUniqueNetIdToRepl(LocalUserId), UPartyId::FromNative(PartyId), ConvertDangerousUniqueNetIdToRepl(SenderId), Platform, UReadablePartyData::FromNative(PartyData));
            })
        );
...
void UOnlinePartySubsystem::BeginDestroy()
{
    ...
    if (Handle.IsValid())
    {
        ...
        Handle->ClearOnPartyJoinRequestReceivedDelegate_Handle(this->DelegateHandle_OnPartyJoinRequestReceived);
        Handle->ClearOnPartyJIPRequestReceivedDelegate_Handle(this->DelegateHandle_OnPartyJIPRequestReceived);
        Handle->ClearOnQueryPartyJoinabilityReceivedDelegate_Handle(this->DelegateHandle_OnQueryPartyJoinabilityReceived);
...
void UOnlinePartySubsystemRejoinParty::Activate()
{
    ...
    if (!Handle->RejoinParty(this->__Store__LocalUserId.GetUniqueNetId().ToSharedRef().Get(), this->__Store__PartyId->ToNative(), FOnlinePartyTypeId(this->__Store__PartyTypeId), ConvertArrayElements<FUniqueNetIdRepl, TSharedRef<const FUniqueNetId>>(this->__Store__FormerMembers, [](const FUniqueNetIdRepl& Val) { return Val.GetUniqueNetId().ToSharedRef(); }), __DelegateHandle_OnJoinPartyComplete))
    {
...
void UOnlinePartySubsystem::RespondToQueryJoinability(FUniqueNetIdRepl LocalUserId, UPartyId* PartyId, FUniqueNetIdRepl RecipientId, bool bCanJoin, int32 DeniedResultCode)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
    ...
    Handle->RespondToQueryJoinability(LocalUserId.GetUniqueNetId().ToSharedRef().Get(), PartyId->ToNative(), RecipientId.GetUniqueNetId().ToSharedRef().Get(), bCanJoin, DeniedResultCode);
}
...
bool UOnlinePartySubsystem::UpdatePartyData(FUniqueNetIdRepl LocalUserId, UPartyId* PartyId, UReadablePartyData* PartyData)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
    ...
    auto __Result = Handle->UpdatePartyData(LocalUserId.GetUniqueNetId().ToSharedRef().Get(), PartyId->ToNative(), PartyData->ToNativeConst().Get());

    return __Result;
}

bool UOnlinePartySubsystem::UpdatePartyMemberData(FUniqueNetIdRepl LocalUserId, UPartyId* PartyId, UReadablePartyData* PartyMemberData)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
    ...
    auto __Result = Handle->UpdatePartyMemberData(LocalUserId.GetUniqueNetId().ToSharedRef().Get(), PartyId->ToNative(), PartyMemberData->ToNativeConst().Get());
...
UReadablePartyData* UOnlinePartySubsystem::GetPartyData(FUniqueNetIdRepl LocalUserId, UPartyId* PartyId)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
    ...
    auto __Result = UReadablePartyData::FromNative(Handle->GetPartyData(LocalUserId.GetUniqueNetId().ToSharedRef().Get(), PartyId->ToNative()));

    return __Result;
}

UReadablePartyData* UOnlinePartySubsystem::GetPartyMemberData(FUniqueNetIdRepl LocalUserId, UPartyId* PartyId, FUniqueNetIdRepl MemberId)
{
    check(!this->HasAnyFlags(RF_ClassDefaultObject));
    auto Handle = this->GetHandle();
    if (!this->IsHandleValid(Handle))
    ...
    auto __Result = UReadablePartyData::FromNative(Handle->GetPartyMemberData(LocalUserId.GetUniqueNetId().ToSharedRef().Get(), PartyId->ToNative(), MemberId.GetUniqueNetId().ToSharedRef().Get()));
...
bool UOnlinePartySubsystem::GetPendingInvitedUsers(FUniqueNetIdRepl LocalUserId, UPartyId* PartyId, TArray<FUniqueNetIdRepl>& OutPendingInvitedUserArray)
{
    ...
    TArray<TSharedRef<const FUniqueNetId>> __StoreTemp__OutPendingInvitedUserArray;
    ...
    OutPendingInvitedUserArray = ConvertArrayElements<TSharedRef<const FUniqueNetId>, FUniqueNetIdRepl>(__StoreTemp__OutPendingInvitedUserArray, [](const TSharedRef<const FUniqueNetId>& Val) { return FUniqueNetIdRepl(Val); });
