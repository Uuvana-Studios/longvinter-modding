...
UOnlineFriendsSubsystemUpdateFriendSettings* UOnlineFriendsSubsystemUpdateFriendSettings::UpdateFriendSettings(UOnlineFriendsSubsystem* Subsystem ,FUniqueNetIdRepl LocalUserId, FFriendSettingsData NewSettings)
{
    if (!IsValid(Subsystem))
    {
        UE_LOG(LogOnlineBlueprint, Error, TEXT("OnlineFriendsSubsystem is not available. It's possible the online subsystem you are using doesn't support it. Alternatively, make sure that you are not attempting to use Online Subsystem Blueprints during the Init event in a GameInstance. Add a 'Delay 0.1' node between the Init event and the rest of your logic if you need to use Online Subsystem Blueprints this early in game startup."));
    ...
    UOnlineFriendsSubsystemUpdateFriendSettings* Node = NewObject<UOnlineFriendsSubsystemUpdateFriendSettings>();
    Node->__Store__Subsystem = Subsystem;
    Node->__Store__CallUniquenessId = TEXT("");
    Node->__Store__LocalUserId = LocalUserId;
    Node->__Store__NewSettings = NewSettings;
    return Node;
}

void UOnlineFriendsSubsystemUpdateFriendSettings::Activate()
{
    auto Handle = this->__Store__Subsystem->GetHandle();
    if (!this->__Store__Subsystem->IsHandleValid(Handle))
    {
    ...
    auto __DelegateHandle_OnSettingsOperationComplete = FOnSettingsOperationComplete::CreateUObject(this, &UOnlineFriendsSubsystemUpdateFriendSettings::HandleCallback_OnSettingsOperationComplete);

    Handle->UpdateFriendSettings(this->__Store__LocalUserId.GetUniqueNetId().ToSharedRef().Get(), this->__Store__NewSettings.ToNative(), __DelegateHandle_OnSettingsOperationComplete);
}

void UOnlineFriendsSubsystemUpdateFriendSettings::HandleCallback_OnSettingsOperationComplete(const FUniqueNetId& UserId, bool bWasSuccessful, bool bWasUpdate, const FFriendSettings& Settings, const FString& ErrorStr)
{
    if (!IsValid(this->__Store__Subsystem))
    {
        UE_LOG(LogOnlineBlueprint, Error, TEXT("Subsystem was not valid during callback handling"));
