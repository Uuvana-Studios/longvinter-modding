...
UOnlinePartySubsystem::UOnlinePartySubsystem()
{
    ...
    if (Handle.IsValid())
    {
        ...
        this->DelegateHandle_OnPartyJIPResponse = Handle->AddOnPartyJIPResponseDelegate_Handle(
            FOnPartyJIPResponse::FDelegate::CreateWeakLambda(this, [this](const FUniqueNetId& LocalUserId, const FOnlinePartyId& PartyId, bool Success, int32 DeniedResultCode) {
                this->OnPartyJIPResponse.Broadcast(ConvertDangerousUniqueNetIdToRepl(LocalUserId), UPartyId::FromNative(PartyId), Success, DeniedResultCode);
            })
        );
...
void UOnlinePartySubsystem::BeginDestroy()
{
    ...
    if (Handle.IsValid())
    {
        ...
        Handle->ClearOnPartyJIPResponseDelegate_Handle(this->DelegateHandle_OnPartyJIPResponse);
