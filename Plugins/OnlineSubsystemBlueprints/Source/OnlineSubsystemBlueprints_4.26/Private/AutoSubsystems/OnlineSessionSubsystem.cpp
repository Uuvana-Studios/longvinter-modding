...
UOnlineSessionSubsystem::UOnlineSessionSubsystem()
{
    ...
    if (Handle.IsValid())
    {
        ...
        this->DelegateHandle_OnSessionUserInviteAccepted = Handle->AddOnSessionUserInviteAcceptedDelegate_Handle(
            FOnSessionUserInviteAccepted::FDelegate::CreateWeakLambda(this, [this](const bool bWasSuccessful, const int32 ControllerId, TSharedPtr<const FUniqueNetId> UserId, const FOnlineSessionSearchResult& InviteResult) {
        ...
        this->DelegateHandle_OnRegisterPlayersComplete = Handle->AddOnRegisterPlayersCompleteDelegate_Handle(
            FOnRegisterPlayersComplete::FDelegate::CreateWeakLambda(this, [this](FName SessionName, const TArray<TSharedRef<const FUniqueNetId>>& PlayerIds, bool bWasSuccessful) {
                this->OnRegisterPlayersComplete.Broadcast(SessionName, ConvertArrayElements<TSharedRef<const FUniqueNetId>, FUniqueNetIdRepl>(PlayerIds, [](const TSharedRef<const FUniqueNetId>& Val) { return FUniqueNetIdRepl(Val); }), bWasSuccessful);
            })
        );
        this->DelegateHandle_OnUnregisterPlayersComplete = Handle->AddOnUnregisterPlayersCompleteDelegate_Handle(
            FOnUnregisterPlayersComplete::FDelegate::CreateWeakLambda(this, [this](FName SessionName, const TArray<TSharedRef<const FUniqueNetId>>& PlayerIds, bool bWasSuccessful) {
                this->OnUnregisterPlayersComplete.Broadcast(SessionName, ConvertArrayElements<TSharedRef<const FUniqueNetId>, FUniqueNetIdRepl>(PlayerIds, [](const TSharedRef<const FUniqueNetId>& Val) { return FUniqueNetIdRepl(Val); }), bWasSuccessful);
            })
        );
...
void UOnlineSessionSubsystemStartMatchmaking::Activate()
{
    ...
    if (!Handle->StartMatchmaking(ConvertArrayElements<FUniqueNetIdRepl, TSharedRef<const FUniqueNetId>>(this->__Store__LocalPlayers, [](const FUniqueNetIdRepl& Val) { return Val.GetUniqueNetId().ToSharedRef(); }), this->__Store__SessionName, this->__Store__NewSessionSettings.ToNative(), this->__Store__SearchSettings->ToNativeRef()))
    {
...
bool UOnlineSessionSubsystem::SendSessionInviteToFriends(FUniqueNetIdRepl LocalUserId, FName SessionName, TArray<FUniqueNetIdRepl> Friends)
{
    ...
    auto __Result = Handle->SendSessionInviteToFriends(LocalUserId.GetUniqueNetId().ToSharedRef().Get(), SessionName, ConvertArrayElements<FUniqueNetIdRepl, TSharedRef<const FUniqueNetId>>(Friends, [](const FUniqueNetIdRepl& Val) { return Val.GetUniqueNetId().ToSharedRef(); }));
...
void UOnlineSessionSubsystemRegisterPlayers::Activate()
{
    ...
    if (!Handle->RegisterPlayers(this->__Store__SessionName, ConvertArrayElements<FUniqueNetIdRepl, TSharedRef<const FUniqueNetId>>(this->__Store__Players, [](const FUniqueNetIdRepl& Val) { return Val.GetUniqueNetId().ToSharedRef(); }), this->__Store__bWasInvited))
    {
...
void UOnlineSessionSubsystemRegisterPlayers::HandleCallback_OnRegisterPlayersComplete(FName SessionName, const TArray<TSharedRef<const FUniqueNetId>>& PlayerIds, bool bWasSuccessful)
{
    if (!IsValid(this->__Store__Subsystem))
    {
        UE_LOG(LogOnlineBlueprint, Error, TEXT("Subsystem was not valid during callback handling"));
...
void UOnlineSessionSubsystemUnregisterPlayers::Activate()
{
    ...
    if (!Handle->UnregisterPlayers(this->__Store__SessionName, ConvertArrayElements<FUniqueNetIdRepl, TSharedRef<const FUniqueNetId>>(this->__Store__Players, [](const FUniqueNetIdRepl& Val) { return Val.GetUniqueNetId().ToSharedRef(); })))
    {
...
void UOnlineSessionSubsystemUnregisterPlayers::HandleCallback_OnUnregisterPlayersComplete(FName SessionName, const TArray<TSharedRef<const FUniqueNetId>>& PlayerIds, bool bWasSuccessful)
{
    if (!IsValid(this->__Store__Subsystem))
    {
        UE_LOG(LogOnlineBlueprint, Error, TEXT("Subsystem was not valid during callback handling"));
