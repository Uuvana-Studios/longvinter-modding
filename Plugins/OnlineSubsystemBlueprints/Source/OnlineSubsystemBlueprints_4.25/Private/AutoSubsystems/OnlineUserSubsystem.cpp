    ...
    if (Handle.IsValid())
    {
        for (auto i = 0; i < MAX_LOCAL_PLAYERS; i++)
        {
            this->DelegateHandle_OnQueryUserInfoComplete.Add(Handle->AddOnQueryUserInfoCompleteDelegate_Handle(
                i,
                FOnQueryUserInfoComplete::FDelegate::CreateWeakLambda(this, [this, i](int32 _UnusedLocalUserNum, bool bWasSuccessful, const TArray<TSharedRef<const FUniqueNetId>>& UserIds, const FString& ErrorStr) {
                    this->OnQueryUserInfoComplete.Broadcast(i, bWasSuccessful, ConvertArrayElements<TSharedRef<const FUniqueNetId>, FUniqueNetIdRepl>(UserIds, [](const TSharedRef<const FUniqueNetId>& Val) { return FUniqueNetIdRepl(Val); }), ErrorStr);
                })
            ));
        }
    }
...
void UOnlineUserSubsystemQueryUserInfo::Activate()
{
    ...
    if (!Handle->QueryUserInfo(this->__Store__LocalUserNum, ConvertArrayElements<FUniqueNetIdRepl, TSharedRef<const FUniqueNetId>>(this->__Store__UserIds, [](const FUniqueNetIdRepl& Val) { return Val.GetUniqueNetId().ToSharedRef(); })))
    {
...
void UOnlineUserSubsystemQueryUserInfo::HandleCallback_OnQueryUserInfoComplete(int32 LocalUserNumCb, bool bWasSuccessful, const TArray<TSharedRef<const FUniqueNetId>>& UserIds, const FString& ErrorStr)
{
    ...
    this->OnQueryUserInfoComplete.Broadcast(bWasSuccessful, ConvertArrayElements<TSharedRef<const FUniqueNetId>, FUniqueNetIdRepl>(UserIds, [](const TSharedRef<const FUniqueNetId>& Val) { return FUniqueNetIdRepl(Val); }), ErrorStr);
}
...
void UOnlineUserSubsystem::GetExternalIdMappings(FExternalIdQueryOptionsBP QueryOptions, TArray<FString> ExternalIds, TArray<FUniqueNetIdRepl>& OutIds)
{
    ...
    TArray<TSharedPtr<const FUniqueNetId>> __StoreTemp__OutIds;
    ...
    OutIds = ConvertArrayElements<TSharedPtr<const FUniqueNetId>, FUniqueNetIdRepl>(__StoreTemp__OutIds, [](const TSharedPtr<const FUniqueNetId>& Val) { return FUniqueNetIdRepl(Val); });
}
